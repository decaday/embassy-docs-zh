...
所以，你想尝试一下Embassy诶，这太棒了！那，首先，你需要安装一些工具：
...
成功构建构建示例后，将你的开发板使用调试器链接到你的电脑上，然后在硬件上运行它：
...
* 已更改了 `+examples/Cargo.toml+` 的 HAL（例如 embassy-stm32）依赖项的特性，以使用正确的芯片（替换默认的 stm32xxxx feature）
